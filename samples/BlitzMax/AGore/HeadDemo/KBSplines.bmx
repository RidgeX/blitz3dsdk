...
Global icv#[nchannels-1+1]				'stores interpolated channel values
Global key0:bbKeyFrame, key1:bbKeyFrame
key0:bbKeyFrame = New bbkeyframe ; key1:bbKeyFrame = New bbkeyframe

Type bbMotion Extends TBBType

	Method New()
		Add(Motion_list)
	End Method

	Method After:bbMotion()
		Local t:TLink
		t=_link.NextLink()
		If t Return bbMotion(t.Value())
	End Method

	Method Before:bbMotion()
		Local t:TLink
		t=_link.PrevLink()
		If t Return bbMotion(t.Value())
	End Method
 ...
 Field keylist:bbKeyFrame[nkeyframes-1]'array of keyframes
End Type

Type bbKeyFrame Extends TBBType

	Method New()
		Add(KeyFrame_list)
	End Method

	Method After:bbKeyFrame()
		Local t:TLink
		t=_link.NextLink()
		If t Return bbKeyFrame(t.Value())
	End Method

	Method Before:bbKeyFrame()
		Local t:TLink
		t=_link.PrevLink()
		If t Return bbKeyFrame(t.Value())
	End Method
 ...
 Field cv#[]=New Float[nchannels]				'array of channel values (x y z h p b sx sy sz)
...
Function Load_Motion( file$, m:bbMotion )
 ...
 m.nkeys = Int(ReadLine$(f))
 
 For i = 0 To m.nkeys									'read keyframe data
 	s$ = ReadLine$( f )
	m.keylist[i] = New bbKeyFrame
...
'------------------------------------------------------------------------
Function Apply_Motion(m:bbMotion, tstep#, e)
 ...
 For key = 1 To m.nkeys						
   If tstep <= m.keylist[key].fstep Then Exit
 Next
 key1 = m.keylist[key]

 If key = m.nkeys Then Return
 ...
 bbPositionEntity e,icv(0),icv(1),icv(2)
 bbRotateEntity e,icv(3),icv(4),icv(5)
 bbScaleEntity e,icv(6),icv(7),icv(8)
