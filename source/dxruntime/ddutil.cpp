...
//	if( !(desc.ddpfPixelFormat.dwFlags & DDSCAPS_MIPMAP) ) return;
	...
	while( src && src->GetAttachedSurface( &caps,&dest )>=0 ){
...
void ddUtil::copy( ddSurf *dest,int dx,int dy,int dw,int dh,ddSurf *src,int sx,int sy,int sw,int sh ){
	...
	DDSURFACEDESC2 src_desc={sizeof(src_desc)};
	if (src->Lock( 0,&src_desc,DDLOCK_WAIT|DDLOCK_READONLY,0 )){
		throw L"chunks";//return;
	}

	DDSURFACEDESC2 dest_desc={sizeof(dest_desc)};
	if (dest->Lock( 0,&dest_desc,DDLOCK_WAIT|DDLOCK_WRITEONLY,0 )){
		throw L"chunks";//return;
	}
...
ddSurf *ddUtil::createSurface( int w,int h,int flags,dxGraphics *gfx ){
...
//
IDirectDrawSurface7 *loadDXTC(const char* filename,dxGraphics *gfx)
{
	HRESULT hr;
	DDSURFACEDESC2 ddsd;
...
ddSurf *ddUtil::loadSurface( const std::string &f,int flags,dxGraphics *gfx ){
...
//	printf("attempting loadpixmap fname=%s\n",fname);fflush(stdout);

	pix=LoadPixmap(fname);
//	pix=CreatePixmap(32,32,PF_BGR888);

	
//	pix=brl_pixmap_LoadPixmap( bbStringFromCString(fname));//f.c_str()) );
//	if (pix==&bbNullObject) return 0;
//	pix=CreatePixmap(32,32,PF_BGR888);//
//	flags=0;
	if (!pix) 
		return 0;

//	printf("loadpixmap success w=%d h=%d format=%d pitch=%d\n",pix->width,pix->height,pix->format,pix->pitch);fflush(stdout);

//	bool trans=(pix->format==PF_RGBA8888)||(pix->format==PF_BGRA8888);
	bool trans=(pix->format==PF_BGRA8888);
//	pix=brl_pixmap_ConvertPixmap( pix,PF_BGRA8888 );
	...
	int pitch=pix->pitch;
	void *bits=pix->pixels;

	ddSurf *src=::createSurface( width,height,pitch,bits,gfx->dirDraw );
	if( !src ){
		RuntimeError("loadsurface::createSurface failed!\n");//fflush(stdout);
		return 0;
	}
...
/*
	if( flags & (dxCanvas::CANVAS_TEX_ALPHA|flags & dxCanvas::CANVAS_TEX_MASK) ){
		if( flags & dxCanvas::CANVAS_TEX_MASK ){
			buildMask( src );
		}else{// if( !trans ){
			buildAlpha( src,(flags & dxCanvas::CANVAS_TEX_RGB)?false:true );
		}
*/
	}else{
	...
	if( !dest ){
		RuntimeError("loadsurface::createdestSurface failed!\n");//fflush(stdout);
		src->Release();
		return 0;
	}

	int t_w=width,t_h=height;
	if( flags & dxCanvas::CANVAS_TEXTURE ) adjustTexSize( &t_w,&t_h,gfx->dir3dDev );

//	copy( dest,0,0,t_w,t_h,src,0,height-1,width,-height );

//	printf("calling copy t_w=%d t_h=%d\n",t_w,t_h);fflush(stdout);

	copy( dest,0,0,t_w,t_h,src,0,0,width,height );
...
//extern "C" BBString* bbStringFromCString( const char *p );

//#include "..\..\freeimage241\source\freeimage.h"

// simon was here
/*
int FreeImage_GetWidth(void*dib){return 0;}
int FreeImage_GetHeight(void*dib){return 0;}
int FreeImage_GetPitch(void*dib){return 0;}
void *FreeImage_GetBits(void*dib){return 0;}
void FreeImage_Unload(void *dib){}
*/
...
static void adjustTexSize( int *width,int *height,IDirect3DDevice7 *dir3dDev ){
    ...
    if( ddDesc.dpcTriCaps.dwTextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL){
		for( w=1;w<*width;w<<=1 ){}
		for( h=1;h<*height;h<<=1 ){}
	}
...
ddSurf *ddUtil::createSurface( int w,int h,int flags,dxGraphics *gfx ){
	...
	int hi=flags & dxCanvas::CANVAS_TEX_HICOLOR?1:0;
	...
	if( flags & dxCanvas::CANVAS_TEX_MASK ){
		desc.dwFlags|=DDSD_PIXELFORMAT;
		desc.ddpfPixelFormat=gfx->texRGBMaskFmt[hi];
	}else if( flags & dxCanvas::CANVAS_TEX_RGB ){
		desc.dwFlags|=DDSD_PIXELFORMAT;
		desc.ddpfPixelFormat=(flags&dxCanvas::CANVAS_TEX_ALPHA)?gfx->texRGBAlphaFmt[hi]:gfx->texRGBFmt[hi];
	}else if( flags & dxCanvas::CANVAS_TEX_ALPHA ){
		desc.dwFlags|=DDSD_PIXELFORMAT;
		desc.ddpfPixelFormat=gfx->texAlphaFmt[hi];
	}else if( flags & dxCanvas::CANVAS_TEXTURE ){
		desc.dwFlags|=DDSD_PIXELFORMAT;
		desc.ddpfPixelFormat=gfx->primFmt;
	}
	if( flags & dxCanvas::CANVAS_TEXTURE ){
		desc.ddsCaps.dwCaps|=DDSCAPS_TEXTURE;
		if( !(flags & dxCanvas::CANVAS_TEX_VIDMEM) ){
			desc.ddsCaps.dwCaps2|=DDSCAPS2_TEXTUREMANAGE;
			if( flags & dxCanvas::CANVAS_TEX_MIPMAP ){
				desc.ddsCaps.dwCaps|=DDSCAPS_MIPMAP|DDSCAPS_COMPLEX;
			}
		}
		if( flags & (dxCanvas::CANVAS_TEX_CUBE) ){
			desc.ddsCaps.dwCaps|=DDSCAPS_COMPLEX;
			desc.ddsCaps.dwCaps2|=DDSCAPS2_CUBEMAP|DDSCAPS2_CUBEMAP_ALLFACES;
		}
		adjustTexSize( (int*)&desc.dwWidth,(int*)&desc.dwHeight,gfx->dir3dDev );
		...
		desc.ddsCaps.dwCaps=DDSCAPS_OFFSCREENPLAIN;
		if( flags & dxCanvas::CANVAS_HIGHCOLOR ){
			desc.dwFlags|=DDSD_PIXELFORMAT;
			desc.ddsCaps.dwCaps|=DDSCAPS_SYSTEMMEMORY;
			desc.ddpfPixelFormat.dwSize=sizeof(DDPIXELFORMAT);
			desc.ddpfPixelFormat.dwFlags=DDPF_RGB|DDPF_ALPHAPIXELS;
			...
			desc.ddpfPixelFormat.dwBBitMask=0x0000ff;
			desc.ddpfPixelFormat.dwRGBAlphaBitMask=0xff000000;
		}else if( flags & dxCanvas::CANVAS_NONDISPLAY ){
			desc.ddsCaps.dwCaps|=DDSCAPS_SYSTEMMEMORY;
		}
	}
	ddSurf *surf;
	int res=gfx->dirDraw->CreateSurface( &desc,&surf,0 );
	if (res<0){	//simon was here try again!
		int ww,hh;
		for( ww=1;ww<w;ww<<=1 ){}
		for( hh=1;hh<h;hh<<=1 ){}
		if (ww<256) ww=256;
		...
		desc.dwWidth=ww;
		desc.dwHeight=hh;
		res=gfx->dirDraw->CreateSurface( &desc,&surf,0 );
	}
