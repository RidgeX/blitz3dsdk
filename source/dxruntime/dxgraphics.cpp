...
dxGraphics::dxGraphics( dxRuntime *rt,IDirectDraw7 *dd,IDirectDrawSurface7 *fs,IDirectDrawSurface7 *bs,bool d3d ):
runtime(rt),dirDraw(dd),dir3d(0),dir3dDev(0),def_font(0){	

	dirDraw->QueryInterface( IID_IDirectDraw,(void**)&ds_dirDraw );

	front_canvas=d_new dxCanvas( this,fs,0 );
	back_canvas=d_new dxCanvas( this,bs,0 );

	front_canvas->cls();
	back_canvas->cls();

	def_font=(dxFont*)loadFont( "courier",12,0 );	//simon was here
...
void dxGraphics::setGamma( int r,int g,int b,float dr,float dg,float db ){
	_gammaRamp.red[r&255]=(int)(dr*257.0f);
	_gammaRamp.green[g&255]=(int)(dg*257.0f);
	_gammaRamp.blue[b&255]=(int)(db*257.0f);
}

void dxGraphics::updateGamma( bool calibrate ){
	if( !_gamma ) return;
	_gamma->SetGammaRamp( calibrate ? DDSGR_CALIBRATE : 0,&_gammaRamp );
}

void dxGraphics::getGamma( int r,int g,int b,float *dr,float *dg,float *db ){
	*dr=_gammaRamp.red[r&255]/257.0f;
	*dg=_gammaRamp.green[g&255]/257.0f;
	*db=_gammaRamp.blue[b&255]/257.0f;
}

void dxGraphics::backup(){
}

bool dxGraphics::restore(){
...
/*
void dxGraphics::backup(){
	set<dxCanvas*>::iterator can_it;
	for( can_it=canvas_set.begin();can_it!=canvas_set.end();++can_it ){
		(*can_it)->backup();
	}
#ifdef PRO
	set<dxMesh*>::iterator msh_it;
	for( msh_it=mesh_set.begin();msh_it!=mesh_set.end();++msh_it ){
		(*msh_it)->backup();
	}
#endif
	if( back_canvas->getSurface()!=front_canvas->getSurface() ) back_canvas->backup();
	front_canvas->backup();
}

void dxGraphics::restore(){
	dirDraw->RestoreAllSurfaces();
	front_canvas->restore();
	if( back_canvas->getSurface()!=front_canvas->getSurface() ) back_canvas->restore();
#ifdef PRO
	set<dxMesh*>::iterator msh_it;
	for( msh_it=mesh_set.begin();msh_it!=mesh_set.end();++msh_it ){
		(*msh_it)->restore();
...
gxCanvas *dxGraphics::getFrontCanvas()const{
	return front_canvas;
}

gxCanvas *dxGraphics::getBackCanvas()const{
	return back_canvas;
}

gxFont *dxGraphics::getDefaultFont()const{
	return def_font;
}

void dxGraphics::vwait(){
	dirDraw->WaitForVerticalBlank( DDWAITVB_BLOCKBEGIN,0 );
}

void dxGraphics::flip( bool v ){
	runtime->flip( v );
}

void dxGraphics::paint( int hwnd ){
	runtime->paint( hwnd);
}

void dxGraphics::copy( gxCanvas *gdest,int dx,int dy,int dw,int dh,gxCanvas *gsrc,int sx,int sy,int sw,int sh ){
	dxCanvas *dest=(dxCanvas *)gdest;	//simon was here
	dxCanvas *src=(dxCanvas *)gsrc;
	RECT r={ dx,dy,dx+dw,dy+dh };
	ddUtil::copy( dest->getSurface(),dx,dy,dw,dh,src->getSurface(),sx,sy,sw,sh );
	dest->damage( r );
}

int dxGraphics::getScanLine()const{
	DWORD t=0;
	dirDraw->GetScanLine( &t );
	return t;
}

int dxGraphics::getTotalVidmem()const{
	DDCAPS caps={sizeof(caps)};
	dirDraw->GetCaps( &caps,0 );
	return caps.dwVidMemTotal;
}

int dxGraphics::getAvailVidmem()const{
	DDCAPS caps={sizeof(caps)};
	dirDraw->GetCaps( &caps,0 );
	return caps.dwVidMemFree;
}

gxMovie *dxGraphics::openMovie( const char *filename,int flags ){
...
// simon was here
	...
	if( CoCreateInstance(
		CLSID_AMMultiMediaStream,NULL,CLSCTX_INPROC_SERVER,IID_IAMMultiMediaStream,(void **)&iam_stream )==S_OK ){
		...
		if( iam_stream->Initialize( STREAMTYPE_READ,AMMSF_NOGRAPHTHREAD,NULL )==S_OK ){
			...
			if( iam_stream->AddMediaStream( ds_dirDraw,&MSPID_PrimaryVideo,0,NULL )==S_OK ){
				...
				if( n==S_OK ){
					dxMovie *movie=d_new dxMovie( this,iam_stream );
...
gxMovie *dxGraphics::verifyMovie( gxMovie *m ){
	return movie_set.count( (dxMovie*)m ) ? m : 0;	//simon was here
}


void dxGraphics::closeMovie( gxMovie *movie ){
// simon was here again - fixed for sdk 0.1
	dxMovie *m=(dxMovie*)movie;
	if( movie_set.erase( m ) ) 
		delete m;
}

gxCanvas *dxGraphics::createCanvas( int w,int h,int flags ){
	ddSurf *s=ddUtil::createSurface( w,h,flags,this );
	if( !s ) return 0;
	dxCanvas *c=d_new dxCanvas( this,s,flags );
	canvas_set.insert( c );
	c->cls();
	return c;
}

gxCanvas *dxGraphics::loadCanvas( const char *filename,int flags ){
...
gxCanvas *dxGraphics::verifyCanvas( gxCanvas *gc ){
	dxCanvas *c=(dxCanvas *)gc;	//simon was here
	return canvas_set.count( c ) || c==front_canvas || c==back_canvas ? c : 0;
}

void dxGraphics::freeCanvas( gxCanvas *gc ){
	dxCanvas *c=(dxCanvas *)gc;	//simon was here
	if( canvas_set.erase( c ) ) delete c;
}

int dxGraphics::getWidth()const{
	return front_canvas->getWidth();
}

int dxGraphics::getHeight()const{
	return front_canvas->getHeight();
}

int dxGraphics::getDepth()const{
	return front_canvas->getDepth();
}


gxFont *dxGraphics::loadFont( const char *fontname,int height,int flags ){
	std::string f=fontname;
	int bold=flags & dxFont::FONT_BOLD ? FW_BOLD : FW_REGULAR;
	int italic=flags & dxFont::FONT_ITALIC ? 1 : 0;
	int underline=flags & dxFont::FONT_UNDERLINE ? 1 : 0;
	int strikeout=0;
	...
	for( int k=0;k<sz;++k ){
		...
		if( GetCharABCWidths( hdc,t,t,&abc ) ){
			if( abc.abcA<0 ){
				as[k]=(int)ceilf(-abc.abcA);
				w+=as[k];
			}
			if( abc.abcC<0 ) w+=(int)ceilf(-abc.abcC);
		}
	...
	if( dxCanvas *c=(dxCanvas *)createCanvas( cw,ch,0 ) ){	//simon was here
		...
		if( surf->GetDC( &surf_hdc )>=0 ){
			...
			c->backup();
			dxFont *font=d_new dxFont( this,c,tm.tmMaxCharWidth,height,first,last+1,tm.tmDefaultChar,offs,widths );
...
gxFont *dxGraphics::verifyFont( gxFont *gf ){
	dxFont *f=(dxFont *)gf;	//simon was here
	return font_set.count( f ) ? f : 0;
}

void dxGraphics::freeFont( gxFont *gf ){
	dxFont *f=(dxFont *)gf;	//simon was here
...
static HRESULT CALLBACK enumDevice( char *desc,char *name,D3DDEVICEDESC7 *devDesc,void *context ){
	dxGraphics *g=(dxGraphics*)context;
	int t=0;
...
static HRESULT CALLBACK enumZbuffFormat( LPDDPIXELFORMAT format,void *context ){
	dxGraphics *g=(dxGraphics*)context;
//	if ((format->dwFlags&DDPF_STENCILBUFFER)==0) return D3DENUMRET_OK;
...
static void pickTexFmts( dxGraphics *g,int hi ){
	//texRGBFmt.
	{
...
gxScene *dxGraphics::createScene( int flags ){
...
#endif
							dxScene *scene=d_new dxScene( this,back_canvas );
...
gxScene *dxGraphics::verifyScene( gxScene *s ){
	return scene_set.count( (dxScene*)s ) ? s : 0;	//simon was here
}

void dxGraphics::freeScene( gxScene *gscene ){
	dxScene *scene=(dxScene *)gscene;	//simon was here
...
gxMesh *dxGraphics::createMesh( int max_verts,int max_tris,int flags ){
/*
	static const int VTXFMT=
	D3DFVF_XYZ|D3DFVF_NORMAL|D3DFVF_DIFFUSE|D3DFVF_TEX2|
	D3DFVF_TEXCOORDSIZE2(0)|D3DFVF_TEXCOORDSIZE2(1);
	D3DVERTEXBUFFERDESC desc={ sizeof(desc),D3DVBCAPS_WRITEONLY,VTXFMT,max_verts };
	IDirect3DVertexBuffer7 *buff;
	if( dir3d->CreateVertexBuffer( &desc,&buff,0 )<0 ) return 0;
	WORD *indices=d_new WORD[max_tris*3];
*/
	dxMesh *mesh=d_new dxMesh( this,max_verts,max_tris );
//	dxMesh *mesh=d_new dxMesh( this,buff,indices,max_verts,max_tris );
	mesh_set.insert( mesh );
	return mesh;
}

gxMesh *dxGraphics::verifyMesh( gxMesh *m ){	//simon was here
	return mesh_set.count( (dxMesh*)m ) ? m : 0;
}

void dxGraphics::freeMesh( gxMesh *mesh ){	//simon was here
//	if( mesh_set.erase( (dxMesh*)mesh ) ) delete mesh; simon was here
	dxMesh *m=(dxMesh*)mesh;
	mesh_set.erase( m );
	m->free();
	delete m;
}
