...
static bool can_wb,can_aa;	//wbuffer,antialias caps
...
//	setRS( D3DRENDERSTATE_NORMALIZENORMALS,FALSE );
	...
	if( dir3dDev->GetCaps( &devDesc )>=0 ){
		...
		caps=devDesc.dpcTriCaps.dwRasterCaps;
		//texture stages
...
void dxScene::setTexState( int n,const TexState &state,bool tex_blend ){
	...
	setTSS( n,D3DTSS_ADDRESSU,(flags & dxCanvas::CANVAS_TEX_CLAMPU) ? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP );
	setTSS( n,D3DTSS_ADDRESSV,(flags & dxCanvas::CANVAS_TEX_CLAMPV) ? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP );

	//texgen
	switch( flags&(
		dxCanvas::CANVAS_TEX_SPHERE|
		dxCanvas::CANVAS_TEX_CUBE) ){

	case dxCanvas::CANVAS_TEX_SPHERE:
		...
		break;
	case dxCanvas::CANVAS_TEX_CUBE:
		switch( state.canvas->cubeMode() & 3 ){
		case dxCanvas::CUBEMODE_NORMAL:
			setTSS( n,D3DTSS_TEXCOORDINDEX,D3DTSS_TCI_CAMERASPACENORMAL );//|tc_index );
			break;
		case dxCanvas::CUBEMODE_POSITION:
	...
	setTSS( n,D3DTSS_ALPHAOP,(flags & dxCanvas::CANVAS_TEX_ALPHA) ? D3DTOP_MODULATE : D3DTOP_SELECTARG2 );
}

int  dxScene::hwTexUnits(){
	return tex_stages;
}
int dxScene::gfxDriverX(){
	return display_x;
}
int dxScene::gfxDriverY(){
	return display_y;
}
int dxScene::gfxDriverHz(){
	return display_hz;
}
int  dxScene::gfxDriverCaps3D(){
	return caps_level;
}

void dxScene::setZMode(){
	switch( zmode ){
	case ZMODE_NORMAL:
	...
	case ZMODE_CMPONLY:
		...
		setRS( D3DRENDERSTATE_ALPHATESTENABLE,false );
		break;
	case ZMODE_ALPHASOLID:
		setRS( D3DRENDERSTATE_ZENABLE,wbuffer ? D3DZB_USEW : D3DZB_TRUE );
		setRS( D3DRENDERSTATE_ZWRITEENABLE,true );
		setRS( D3DRENDERSTATE_ALPHATESTENABLE,true );
		setRS( D3DRENDERSTATE_ALPHAFUNC,D3DCMP_GREATER );
		break;
	case ZMODE_ALPHATRANS:
		setRS( D3DRENDERSTATE_ZENABLE,wbuffer ? D3DZB_USEW : D3DZB_TRUE );
		setRS( D3DRENDERSTATE_ZWRITEENABLE,false );
		setRS( D3DRENDERSTATE_ALPHATESTENABLE,true );
		setRS( D3DRENDERSTATE_ALPHAFUNC,D3DCMP_LESSEQUAL );
		break;
	}
}

void dxScene::setLights(){
	if( fx & FX_FULLBRIGHT ){
		//no lights on
...
void dxScene::setHWMultiTex( bool e ){
	for( int n=0;n<8;++n ){
...
void dxScene::setWBuffer( bool n ){
	if( n==wbuffer || !can_wb ) return;
	wbuffer=n;setZMode();
}

void dxScene::setDither( bool n ){
	if( n==dither ) return;
	dither=n;setRS( D3DRENDERSTATE_DITHERENABLE,dither ? true : false );
}

void dxScene::setAntialias( int level ){
//	if( n==antialias ) return;

	D3DDEVICEDESC7 devDesc={0};
	if( dir3dDev->GetCaps( &devDesc )>=0 ){
		DWORD caps=devDesc.dpcTriCaps.dwRasterCaps;
		if( (caps & D3DPRASTERCAPS_ANTIALIASEDGES)) can_aa=true;
		if( (caps & D3DPRASTERCAPS_ANTIALIASSORTINDEPENDENT)) can_aa=true;
		if( (caps & D3DPRASTERCAPS_ANTIALIASSORTDEPENDENT)) can_aa=true;
	}

	if (can_aa){
		antialias=level;
		if (antialias){
			setRS( D3DRENDERSTATE_ANTIALIAS,D3DANTIALIAS_SORTDEPENDENT  );
			if (level>1){
				setRS( D3DRENDERSTATE_EDGEANTIALIAS , true );
			}
		}else{
			setRS( D3DRENDERSTATE_ANTIALIAS,D3DANTIALIAS_NONE );
		}
	}else{
		antialias=0;
	}
...
void dxScene::setOrthoProj( float nr,float fr,float w,float h ){
...
void dxScene::setPerspProj( float nr,float fr,float w,float h ){
	if( !ortho_proj && nr==frustum_nr && fr==frustum_fr && w==frustum_w && h==frustum_h ) return;
...
void dxScene::setFogColor( const float rgb[3] ){
	int n=(int(rgb[0]*255.0f)<<16)|(int(rgb[1]*255.0f)<<8)|int(rgb[2]*255.0f);
	if( n==fogcolor ) return;
	fogcolor=n;setRS( D3DRENDERSTATE_FOGCOLOR,fogcolor );
}

void dxScene::setFogRange( float nr,float fr ){
...
void dxScene::setFogMode( int n ){
	if( n==fogmode ) return;
	fogmode=n;setFogMode();
}

void dxScene::setZMode( int n ){
	if( n==zmode ) return;
	zmode=n;setZMode();
}

void dxScene::setViewMatrix( const Matrix *m ){
	if( m ){
...
void dxScene::setWorldMatrix( const Matrix *m ){
	if( m ){
...
void dxScene::setRenderState( const RenderState &rs ){
	bool setmat=false;
	...
	if( rs.alpha!=material.diffuse.a ){
		...
		if( rs.fx&FX_ALPHATEST ){
			int alpharef=(rs.fx&FX_VERTEXALPHA)?0:int(ALPHAREF*rs.alpha);
			setRS( D3DRENDERSTATE_ALPHAREF,alpharef );
		}
	...
	for( int k=0;k<MAX_TEXTURES;++k ){
		...
		dxCanvas *tscanvas=(dxCanvas*)ts.canvas;	//simon was here
		tscanvas->getTexSurface();	//force mipmap rebuild
		if( tscanvas!=hw->canvas ){ hw->canvas=tscanvas;settex=true; }
...
void dxScene::begin( int lightcount,gxLight **lightlist ){	//const vector<gxLight*> &lights ){
	...
	for( n=0;n<8;++n ){
		if( n<lightcount ){
			_curLights.push_back( (dxLight*)lightlist[n] );	////simon was here
...
/*
		if( n<lights.size() ){
			_curLights.push_back( (dxLight*)lights[n] );	////simon was here
			dir3dDev->SetLight( n,&_curLights[n]->d3d_light );
		}else{
			dir3dDev->LightEnable( n,false );
		}
...
void dxScene::clear( const float rgb[3],float alpha,float z,bool clear_argb,bool clear_z ){
...
void dxScene::render( gxMesh *gm,int first_vert,int vert_cnt,int first_tri,int tri_cnt ){
	dxMesh *m=(dxMesh *)gm;	//simon was here
...
gxLight *dxScene::createLight( int flags ){
	dxLight *l=d_new dxLight( this,flags );
	_allLights.insert(l);
	return l;
}

void dxScene::freeLight( gxLight *l ){
	_allLights.erase((dxLight*)l);	//simon was here
}

int dxScene::getTrianglesDrawn()const{
	return tris_drawn;
}
