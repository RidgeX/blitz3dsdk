...
class dxGraphics;
typedef struct IDirectDrawSurface7 ddSurf;
...
struct ddUtil{
	...
	static ddSurf *loadSurface( const std::string &f,int flags,dxGraphics *gfx );
	static ddSurf *createSurface( int width,int height,int flags,dxGraphics *gfx );
};
...
class PixelFormat{
	...
	Plot plot;
	Point point;
//	char plot_code[64],point_code[64];
...
public:
	...
	PixelFormat( const DDPIXELFORMAT &pf ){
		plot_code=0;
		point_code=0;
		setFormat( pf );
	}
