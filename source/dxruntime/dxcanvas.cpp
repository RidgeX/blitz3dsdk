...
//#define DEBUG_BITMASK bad linkage removed...
...
dxCanvas::dxCanvas( dxGraphics *g,IDirectDrawSurface7 *s,int f ):
...
t_surf(0),cm_mask(0),locked_cnt(0),mod_cnt(0),remip_cnt(0){
	...
	if( flags & CANVAS_TEX_CUBE ){
		...
		for( int k=0;k<6;++k ){
			...
			res=main_surf->GetAttachedSurface( &caps,&cube_surfs[k] );
		}
		surf=cube_surfs[1];
	}else{
		surf=main_surf;
	...
	setOrigin( 0,0 );
	setHandle( 0,0 );
	setFont( (dxFont*)graphics->getDefaultFont() );	//simon was here
	setViewport( 0,0,getWidth(),getHeight() );
	if( flags & dxCanvas::CANVAS_TEXTURE ) ddUtil::buildMipMaps( surf );
}
...
void dxCanvas::backup()const{
	...
	if( graphics->dirDraw->CreateSurface( &t_desc,&t_surf,0 )<0 ) return;
	t_surf->Blt( 0,surf,0,DDBLT_WAIT,0 );
}
...
void dxCanvas::restore()const{
	...
	if( !t_surf ) return;
	surf->Blt( 0,t_surf,0,DDBLT_WAIT,0 );
	t_surf->Release();
	t_surf=0;
}
*/

ddSurf *dxCanvas::getSurface()const{
	return surf;
}

ddSurf *dxCanvas::getTexSurface()const{
...
bool dxCanvas::clip( RECT *d )const{
	return ::clip( viewport,d );
}

bool dxCanvas::clip( RECT *d,RECT *s )const{
	return ::clip( viewport,d,s );
}

void dxCanvas::updateBitMask( const RECT &r )const{
	...
	while( h-- ){
		...
		for( int c=0;c<w;++c ){
			...
			if( dest<cm_mask || dest>=cm_mask_end ){
				gx_runtime->debugError( "dxCanvas::updateBitMask dest out of range" );
			}
#endif
			*dest++=mask;
		}
...
void dxCanvas::setModify( int n ){
	mod_cnt=n;
}

int dxCanvas::getModify()const{
	return mod_cnt;
}

bool dxCanvas::attachZBuffer(){
...
void dxCanvas::damage( const RECT &r )const{
	++mod_cnt;if( cm_mask ) updateBitMask( r );
}

void dxCanvas::setFont( gxFont *f ){
	font=(dxFont*)f;	//simon was here
}

void dxCanvas::setMask( unsigned argb ){
	mask_surf=format.fromARGB( argb );
}

void dxCanvas::setColor( unsigned argb ){
	argb|=0xff000000;
	color_argb=argb;
	color_surf=format.fromARGB( argb );
}

void dxCanvas::setClsColor( unsigned argb ){
	argb|=0xff000000;
	clsColor_surf=format.fromARGB( argb );
}

void dxCanvas::setOrigin( int x,int y ){
	origin_x=x;origin_y=y;
}

void dxCanvas::setHandle( int x,int y ){
	handle_x=x;handle_y=y;
}

void dxCanvas::setViewport( int x,int y,int w,int h ){
	Rect r( x,y,w,h );
...
void dxCanvas::plot( int x,int y ){
...
void dxCanvas::line( int x0,int y0,int x1,int y1 ){
	int ddf,padj,sadj;
...
void dxCanvas::rect( int x,int y,int w,int h,bool solid ){
...
void dxCanvas::oval( int x1,int y1,int w,int h,bool solid ){
	x1+=origin_x;y1+=origin_y;
	Rect dest( x1,y1,w,h );
	if( !clip( &dest ) ) return;
...
void dxCanvas::blit( int x,int y,gxCanvas *gsrc,int src_x,int src_y,int src_w,int src_h,bool solid ){
	dxCanvas *src=(dxCanvas *)gsrc;	//simon was here
...
void dxCanvas::text( int x,int y,const char *txt){//string &t ){
...
int dxCanvas::getWidth()const{
	return clip_rect.right;
}

int dxCanvas::getHeight()const{
	return clip_rect.bottom;
}

int dxCanvas::getDepth()const{
	return format.getDepth();
}

void dxCanvas::getOrigin( int *x,int *y )const{
	*x=origin_x;*y=origin_y;
}

void dxCanvas::getHandle( int *x,int *y )const{
	*x=handle_x;*y=handle_y;
}

void dxCanvas::getViewport( int *x,int *y,int *w,int *h )const{
	*x=viewport.left;*y=viewport.top;
	*w=viewport.right-viewport.left;*h=viewport.bottom-viewport.top;
}

unsigned dxCanvas::getMask()const{
	return format.toARGB( mask_surf );
}

unsigned dxCanvas::getColor()const{
	return format.toARGB( color_surf );
}

unsigned dxCanvas::getClsColor()const{
	return format.toARGB( clsColor_surf );
}

bool dxCanvas::collide( int x1,int y1,const gxCanvas *ii2,int x2,int y2,bool solid )const{

	const dxCanvas *i2=(dxCanvas*)ii2;	//simon was here
	...
	for( int y=ir.top;y<ir.bottom;++y ){
		...
		for( int x=0;x<cnt;++x ){
			...
			if( row1<i1->cm_mask || row2<i2->cm_mask ){
				gx_runtime->debugError( "dxCanvas::collide row underflow" );
			}
			if( row1>=cm_mask_end1 || row2>=cm_mask_end2 ){
				gx_runtime->debugError( "dxCanvas::collide row overflow" );
			}
#endif
			unsigned n=*row2++;
...
bool dxCanvas::rect_collide( int x1,int y1,int x2,int y2,int w2,int h2,bool solid )const{
...
void dxCanvas::setPixel( int x,int y,unsigned argb ){
	x+=origin_x;if( x<viewport.left || x>=viewport.right ) return;
	y+=origin_y;if( y<viewport.top || y>=viewport.bottom ) return;
	lock();
...
unsigned dxCanvas::getPixel( int x,int y )const{
...
void dxCanvas::copyPixelFast( int x,int y,gxCanvas *gsrc,int src_x,int src_y ){
	dxCanvas *src=(dxCanvas*)gsrc;	//simon was here
...
void dxCanvas::copyPixel( int x,int y,gxCanvas *gsrc,int src_x,int src_y ){
...
void dxCanvas::setCubeMode( int mode ){
	cube_mode=mode;
}

void dxCanvas::setCubeFace( int face ){
