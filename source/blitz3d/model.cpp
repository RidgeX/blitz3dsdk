...
//		q_type=(n==gxScene::BLEND_REPLACE) ? QUEUE_OPAQUE : QUEUE_TRANSPARENT;
		q_type=(n==gxScene::BLEND_REPLACE) ? QUEUE_OPAQUE : ((n==gxScene::BLEND_ALPHA)?QUEUE_ALPHA:QUEUE_TRANSPARENT);
	}
...
bool Model::doAutoFade( const Vector &eye ){
	...
	if( alpha!=render_brush.getAlpha() ) render_brush.setAlpha( alpha );
	else if( !w_brush ) return true;
...
void Model::preRenderQueue( int type ){
	vector<MeshQueue*> que=queues[type];
	for( ;que.size();que.pop_back() ){
		MeshQueue *q=que.back();
		q->render();
