...
inline Matrix pitchMatrix( float q ){
	return Matrix( Vector(1,0,0),Vector(0,cos(q),sin(q)),Vector(0,-sin(q),cos(q)) );
}

inline Matrix yawMatrix( float q ){
	return Matrix( Vector(cos(q),0,sin(q)),Vector(0,1,0),Vector(-sin(q),0,cos(q)) );
}

inline Matrix rollMatrix( float q ){
	return Matrix( Vector(cos(q),sin(q),0),Vector(-sin(q),cos(q),0),Vector(0,0,1) );
}
...
inline Quat pitchQuat( float p ){
	return Quat( cos(p/-2),Vector( sin(p/-2),0,0 ) );
}

inline Quat yawQuat( float y ){
	return Quat( cos(y/2),Vector( 0,sin(y/2),0 ) );
}

inline Quat rollQuat( float r ){
	return Quat( cos(r/-2),Vector( 0,0,sin(r/-2) ) );
}
