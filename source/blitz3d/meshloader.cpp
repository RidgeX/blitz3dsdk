...
struct MLMesh{
	...
	~MLMesh(){
		map<Brush,xSurf*>::const_iterator it;
...
void MeshLoader::addTriangle( int v0,int v1,int v2,const Brush &b ){
	//find surface
	xSurf *surf;
	map<Brush,xSurf*>::const_iterator it=ml_mesh->brush_map.find( b );
	if( it!=ml_mesh->brush_map.end() ){
		surf=it->second;
	}else{
		surf=d_new xSurf;
		pair<Brush,xSurf*> p=make_pair( b,surf );
		ml_mesh->brush_map.insert(p);
	}
/*		
