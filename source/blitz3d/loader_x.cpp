...
HMODULE d3dxoflib=0;		//LoadLibrary("d3dxof");
int (__stdcall *directxfilecreate) (LPDIRECTXFILE *lplpDirectXFile)=0;

int linkxlib(){	//true=notfount
	if (directxfilecreate) return 0;
	if (d3dxoflib==0) d3dxoflib=LoadLibrary("d3dxof");
	if (d3dxoflib==0) return 1;	
	directxfilecreate=(int(__stdcall *)(LPDIRECTXFILE *lplpDirectXFile))GetProcAddress(d3dxoflib,"DirectXFileCreate");
	if (directxfilecreate==0) return 1;
	return 0;
}
			...
			if( n==4 ){
				...
				if( conv ){
					if( fabs(rot.w)<1-EPSILON ){
						...
						rot=Quat( cos( half ),(conv_tform.m*rot.v).normalized()*sin( half ) );
					}else rot=Quat();
				}
		...
		if( childObj->QueryInterface( IID_IDirectXFileData,(void**)&childData )>=0 ){
			if( childData->GetType( &guid )>=0 ){
				if( *guid==TID_D3DRMMaterial ){
					Brush b=parseMaterial( childData );
					mats.push_back( b );
				}
            }
			childData->Release();
...
//							v.color=Vector( t[0],t[1],t[2] );
	...
	for( k=0;k<faces.size();++k ){
		...
		int tri[3];
		tri[0]=data[0];
//		printf("addface cnt=%d f.mat_index=%d mats.size=%d\n",cnt,f.mat_index,mats.size());
		for( int j=2;j<cnt;++j ){
...
extern int (__stdcall *directxfilecreate) (LPDIRECTXFILE *lplpDirectXFile);	//linked from gxruntime.cpp
