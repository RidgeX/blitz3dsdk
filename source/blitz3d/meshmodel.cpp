...
void MeshModel::render( const RenderContext &rc ){
	...
	if( !model_frustum.cull( b ) ) return;
	...
	static vector<Surface::Bone> surf_bones;
	if( bones.size()>surf_bones.size() ) surf_bones.resize( bones.size() );
