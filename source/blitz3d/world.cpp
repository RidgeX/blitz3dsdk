...
//	char buffer[256];
//	sprintf(buffer,"t=%d d=%d m=%d r=%d\n",src_type,dst_type,method,response);
//	RTEX(buffer);//"COLLISIONS!");
...
struct BrushComp{
	bool operator()( Model *a,Model *b ){
		int t1=a->getBrush().getID();
		int t2=b->getBrush().getID();
		return t1<t2;
	}
};
...
static priority_queue<Model*,vector<Model*>,BrushComp> unord_que;

static priority_queue<Model*,vector<Model*>,OrderComp> ord_que;

static priority_queue<Camera*,vector<Camera*>,OrderComp> cam_que;

static priority_queue<Model*,vector<Model*>,TransComp> transparent_que;

static priority_queue<Model*,vector<Model*>,BrushComp> alpha_que;
...
void World::render( float tween ){
	...
	for( ;unord_que.size();unord_que.pop() ) unord_mods.push_back( unord_que.top() );
	...
	if (n){
		l=new gxLight *[n];
		for (int i=0;i<n;i++) l[i]=_lights[i];
	}
	gx_scene->begin( n,l);//_lights );
	delete l;
...
void World::render( Camera *cam,Mirror *mirror ){
	...
	flushAlpha( true );
	gx_scene->setZMode( gxScene::ZMODE_CMPONLY ); //simon was here
	flushTransparent();
...
void World::render( Model *mod,const RenderContext &rc ){
	...
	if( mod->queueSize( Model::QUEUE_TRANSPARENT ) ){
		transparent_que.push( mod );
	}
	if( mod->queueSize( Model::QUEUE_ALPHA ) ){
		alpha_que.push( mod );
	}
}
...
void World::flushTransparent(){
	bool local=true;
	for( ;transparent_que.size();transparent_que.pop() ){
		Model *mod=transparent_que.top();
		if( mod->getRenderSpace()==Model::RENDER_SPACE_LOCAL ){
			gx_scene->setWorldMatrix( (gxScene::Matrix*)&mod->getRenderTform() );
			local=true;
		}else if( local ){
...
void World::flushAlpha( int writez ){
	bool local=true;
	for( ;alpha_que.size();alpha_que.pop() ){
		Model *mod=alpha_que.top();
		if( mod->getRenderSpace()==Model::RENDER_SPACE_LOCAL ){
			gx_scene->setWorldMatrix( (gxScene::Matrix*)&mod->getRenderTform() );
			local=true;
		}else if( local ){
		...
		if ( writez ){
			gx_scene->setZMode( gxScene::ZMODE_ALPHATRANS);
			mod->preRenderQueue( Model::QUEUE_ALPHA );
			gx_scene->setZMode( gxScene::ZMODE_ALPHASOLID);
		}
		mod->renderQueue( Model::QUEUE_ALPHA );
	}
}
