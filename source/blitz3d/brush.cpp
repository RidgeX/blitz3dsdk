...
//	if( rep->texs[0].getCanvasFlags() & (gxCanvas::CANVAS_TEX_MASK|gxCanvas::CANVAS_TEX_ALPHA) ) {//simon was here
...
//			rs.fx&=~gxScene::FX_ALPHATEST;	//simon was here
			return rs.blend=rep->blend;
		}
		for( int k=0;k<rep->max_tex;++k ){
			if( rep->texs[k].isTransparent() ) return rs.blend=gxScene::BLEND_ALPHA;
		}
