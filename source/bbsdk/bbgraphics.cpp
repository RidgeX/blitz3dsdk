...
static inline void debugCanvasXY( gxCanvas *c,int x,int y ){
	if( debug_mode ){
		if( !gx_graphics->verifyCanvas( c ) ) RTEX( "Buffer does not exist" );
		if (x<0||x>=c->getWidth()) RTEX( "Illegal Buffer Coordinate" );
		if (y<0||y>=c->getHeight()) RTEX( "Illegal Buffer Coordinate" );
	}
}
...
/*
const char *	bbGfxDriverName( int n ){
	debugDriver( n );
	string t;int caps;
	gx_runtime->graphicsDriverInfo( n-1,&t,&caps );
	return const charFromCString(t.c_str());
}
const wchar_t *bbGfxDriverWName( int n ){
	debugDriver( n );
	string t;int caps;
	gx_runtime->graphicsDriverInfo( n-1,&t,&caps );
//	return t.w_str();
...
void bbGfxDriverStrName( int n,string &str ){
	debugDriver( n );
	int caps;
	gx_runtime->graphicsDriverInfo( n-1,&str,&caps );
}
*/
...
//	static string t;
	const char *t;
	int caps,x,y,hz;
	gx_runtime->graphicsDriverInfo( n-1,&t,&caps,&x,&y,&hz );
	return t;	//.c_str();
}

int	  bbGfxDriverX(int n){
	debugDriver( n );
	const char *t;
	int caps,x,y,hz;
	gx_runtime->graphicsDriverInfo( n-1,&t,&caps,&x,&y,&hz );
...
int bbLoadBuffer( gxCanvas *c,const char *str ){
	debugCanvas( c );
	gxCanvas *t=gx_graphics->loadCanvas( str,0 );
	if( !t ) return 0;
	float m[2][2];
...
int bbSaveBuffer( gxCanvas *c,const char *str ){
	debugCanvas( c );
	string t=str;
	return saveCanvas( c,t ) ? 1 : 0;
}

void bbAutoBufferDirty( int enable ){
	auto_dirty=enable;
}
...
//3=windowed scaled
//4=windowed reused
//5=windowed scaled reused (currently N/A)
//6=windowed autosuspend
//7=windowed scaled autosuspend
//
void bbGraphics( int w,int h,int d,int mode ){
	int flags=0;
	switch( mode ){
	case 0:flags|=debug_mode ? gxGraphics::GRAPHICS_WINDOWED : 0 ;break;
	case 1:break;
	...
	case 4:flags|=gxGraphics::GRAPHICS_WINDOWED|gxGraphics::GRAPHICS_REUSED;break;
...
void bbGraphics3D( int w,int h,int d,int mode ){
	...
	switch( mode ){
	case 0:flags|=(debug_mode && bbWindowed3D()) ? gxGraphics::GRAPHICS_WINDOWED : 0 ;break;
	case 1:break;
	case 2:flags|=gxGraphics::GRAPHICS_WINDOWED;break;
	case 3:flags|=gxGraphics::GRAPHICS_WINDOWED|gxGraphics::GRAPHICS_SCALED;break;
	case 4:flags|=gxGraphics::GRAPHICS_WINDOWED|gxGraphics::GRAPHICS_REUSED;break;
...
void bbLockBuffer( gxCanvas *buff ){
	...
	buff ? buff->lock() : gx_canvas->lock();
}

void bbUnlockBuffer( gxCanvas *buff ){
	if( buff ) debugCanvas( buff );
	buff ? buff->unlock() : gx_canvas->unlock();
}

int bbReadPixel( int x,int y,gxCanvas *buff ){
	if (!buff) buff=gx_canvas;
	debugCanvasXY( buff,x,y );
...
void bbCopyPixel( int src_x,int src_y,gxCanvas *src,int dest_x,int dest_y,gxCanvas *buff ){
	if (!buff) buff=gx_canvas;
	if (!src) src=gx_canvas;
	debugCanvasXY( src,src_x,src_y );
	debugCanvasXY( buff,dest_x,dest_y );
	buff->copyPixel(dest_x,dest_y,src,src_x,src_y);
...
void bbCopyPixelFast( int src_x,int src_y,gxCanvas *src,int dest_x,int dest_y,gxCanvas *buff ){
	...
	debugCanvasXY( src,src_x,src_y );
	debugCanvasXY( buff,dest_x,dest_y );
	buff->copyPixelFast(dest_x,dest_y,src,src_x,src_y);
...
void bbPaint( int hwnd ){
/*	RECT	rect;
	int		remake;

	remake=0;
	GetClientRect( (HWND)hwnd,&rect );
	if (rect.right>gwidth) {gwidth=rect.right;remake=true;}
	if (rect.bottom>gheight) {gheight=rect.bottom;remake=true;}
	if (remake)
	{
		bbEndGraphics();
		bbGraphics( gwidth,gheight,0,4 );
	}
*/
	gx_graphics->paint( hwnd );
//	if( !gx_runtime->idle() ) RTEX( 0 );
}
...
void bbText( int x,int y,const char *t,int centre_x,int centre_y ){
	if( centre_x ) x-=curr_font->getWidth( t )/2;
	if( centre_y ) y-=curr_font->getHeight()/2;
	gx_canvas->text( x,y,t );
}

/*
void bbText( int x,int y,const std::string &t,int centre_x,int centre_y ){
	if( centre_x ) x-=curr_font->getWidth( t.c_str() )/2;
	if( centre_y ) y-=curr_font->getHeight()/2;
	gx_canvas->text( x,y,t );
}
void const charText( int x,int y,const char *str,int centre_x,int centre_y ){
	if( centre_x ) x-=curr_font->getWidth( str )/2;		//t.c_str()
	if( centre_y ) y-=curr_font->getHeight()/2;
...
gxFont *bbLoadFont( const char *name,int height,int bold,int italic,int underline ){
	int flags=
...
int bbStringWidth( const char *str ){
	return curr_font->getWidth( str );
}

int bbStringHeight( const char *str ){
	return curr_font->getHeight();
}

gxMovie *bbOpenMovie( const char *s ){
...
bbImage *bbLoadImage( const char *s ){
//	string t=s;
	gxCanvas *c=gx_graphics->loadCanvas( s,0 );
	if( !c ) return 0;
	if( auto_dirty ) c->backup();
...
bbImage *bbLoadAnimImage( const char *s,int w,int h,int first,int cnt ){
...
//	string t=s;
	...
	gxCanvas *pic=gx_graphics->loadCanvas( s,gxCanvas::CANVAS_NONDISPLAY );
	if( !pic ) return 0;
...
int bbSaveImage( bbImage *i,const char *str,int n ){
	debugImage( i,n );
	string t=str;
...
void bbWrite( const char *str ){
	gxCanvas *c=startPrinting();
	c->text( curs_x,curs_y,str ); //*str );
	curs_x+=curr_font->getWidth( str );
	endPrinting( c );
}

void bbPrint( const char *str ){
	gxCanvas *c=startPrinting();
...
const char *bbInput( const char *prompt ){
	gxCanvas *c=startPrinting();
//	string t=prompt;//delete prompt;
	...
	while( go ){
		...
		int cx=curs_x+curr_font->getWidth( str.substr( 0,curs ).c_str() );
		int cw=curr_font->getWidth( curs<str.size() ? str.substr( curs,1 ).c_str() : "X" );
		...
		while( gx_runtime->idle() ){
			...
			if( n!=tc ){
				...
				c->text( cx,curs_y,str.substr( curs,1 ).c_str() );
			}
		...
		p_canvas->text( curs_x,curr_font->getHeight(),str.c_str() );
	...
	curs_y+=curr_font->getHeight();
	endPrinting( c );
	return str.c_str();	//d_new BBStr( str );
}
...
void graphics_link( void (*rtSym)( const char *sym,void *pc ) ){
	...
	rtSym( "%StringWidth$string",(void*)const charWidth );
	rtSym( "%StringHeight$string",(void*)const charHeight );
