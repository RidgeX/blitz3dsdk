...
//		MessageBox( GetDesktopWindow(),"mem_check: pre_tag!='DNEW'","Memory error",MB_OK|MB_ICONWARNING );
		printf("Memory error mem_check: pre_tag!='DNEW'\n");
		if( m->tag=='NDWE' ){
...
string itoa( int n ){
	char buff[32];
	sprintf(buff,"%d",n);	//itoa( n,buff,10 );
	return string( buff );
}

int _finite( double n ){		// definition: exponent anything but 2047.
...
int _isnan( double n ){		// definition: exponent 2047, nonzero fraction.
...
int GetFullPathName( const char *filename,int bufferlength,char *buffer,char **filepart)
{
	char	*res,*t;
	res=realpath(filename,buffer);
	if (res)
	{
		t=strrchr(res,'/');
		if (t) *filepart=t+1;else *filepart=res;
